...
public class TezCrypt_Hand_FileSign extends TezCrypt_Hand_BaseSign<File> {
    ...
    TezCrypt_Hand_FileSign(TezCom_Proc<?, ?> previous) {
        super(previous);
    }
    ...
    byte[] onProcess(File input) throws Exception {
        ...
        if (nio) {
            return BaseCipher.sign(input, privateKey, type);
        } else {
            return BaseCipher.signIo(input, privateKey, type);
        }
    }
